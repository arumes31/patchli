...
func sendWebhook(targetURL string, payload WebhookPayload) {
	...
	req, err := http.NewRequest("POST", targetURL, bytes.NewBuffer(data)) // #nosec G704 -- SSRF is mitigated by isValidURL
	if err != nil {
	...
	resp, err := client.Do(req) // #nosec G704 -- SSRF is mitigated by isValidURL
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		log.Printf("Webhook returned error status: %d", resp.StatusCode) // #nosec G706 -- resp.StatusCode is an integer
	}
}
