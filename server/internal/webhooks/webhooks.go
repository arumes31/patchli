...
func isValidURL(u string) bool {
	...
	// 🛡️ Sentinel: Block explicit localhost references to prevent SSRF (net.ParseIP("localhost") is nil)
	if host == "localhost" || host == "127.0.0.1" || host == "::1" {
		return false
	}

	ip := net.ParseIP(host)
	// 🛡️ Sentinel: Block private/internal IPs to prevent SSRF against internal network
