...
func sendWebhook(targetURL string, payload WebhookPayload) {
	...
	req, err := http.NewRequest("POST", targetURL, bytes.NewBuffer(data)) // #nosec G107 -- URL is provided via environment variable configuration, assumed trusted.
	if err != nil {
	...
	resp, err := client.Do(req) // #nosec G107 -- URL is provided via environment variable configuration, assumed trusted.
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		log.Printf("Webhook returned error status: %d", resp.StatusCode) // #nosec G117 -- Status code is an integer and safe to log.
	}
}
