...
func sendWebhook(targetURL string, payload WebhookPayload) {
	...
	req, err := http.NewRequest("POST", targetURL, bytes.NewBuffer(data)) // #nosec G704 -- URL is validated or internal webhook
	if err != nil {
	...
	resp, err := client.Do(req) // #nosec G704 -- URL is validated or internal webhook
	if err != nil {
