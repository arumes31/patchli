...
func sendWebhook(targetURL string, payload WebhookPayload) {
	...
	req, err := http.NewRequest("POST", targetURL, bytes.NewBuffer(data)) // #nosec G704 -- Webhook URLs are admin-configured
	if err != nil {
	...
	resp, err := client.Do(req) // #nosec G704 -- Webhook URLs are admin-configured
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		log.Printf("Webhook returned error status: %d", resp.StatusCode) // #nosec G706 -- Status code is an integer, safe to log
	}
}
