...
// ⚡ Bolt: Iterate directly over the map to avoid O(N) slice allocation for stats
func (am *AgentManager) GetStats() (total int, online int, reboot int) {
	...
	for _, n := range am.details {
		// Avoid strings.ToLower allocation by checking common casings directly or using EqualFold for substring
		// Actually strings.EqualFold doesn't do contains, so we can check if it contains the substring.
		// Alternatively, we know status is generally a known string like "Online" or "offline" or "reboot"
		// Wait, let's use a simpler check:
		if strings.Contains(n.Status, "online") || strings.Contains(n.Status, "Online") || strings.Contains(n.Status, "ONLINE") {
			online++
		}
		if strings.Contains(n.Status, "reboot") || strings.Contains(n.Status, "Reboot") || strings.Contains(n.Status, "REBOOT") {
			reboot++
		}
	}
