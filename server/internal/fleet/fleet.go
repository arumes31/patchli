...
// ⚡ Bolt: GetStats calculates node stats directly from the map, avoiding an O(N) slice allocation and string allocation
func (am *AgentManager) GetStats() (total int, online int, rebootRequired int) {
	am.mu.RLock()
	defer am.mu.RUnlock()
	total = len(am.details)
	for _, n := range am.details {
		// Use strings.EqualFold or direct Contains without ToLower if possible
		// To replicate `Contains(ToLower, "online")` safely and efficiently without allocations:
		if strings.Contains(n.Status, "Online") || strings.Contains(n.Status, "online") || strings.EqualFold(n.Status, "online") {
			online++
		}
