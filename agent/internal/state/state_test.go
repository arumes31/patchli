...
func TestSaveStateError(t *testing.T) {
	...
	// Create a file where a directory should be to trigger MkdirAll error
	collisionPath := filepath.Join(tmpDir, "collision")
	if err := os.WriteFile(collisionPath, []byte("file"), 0644); err != nil {
		t.Fatalf("Failed to create collision file: %v", err)
	}

	oldStateFile := stateFile
	stateFile = filepath.Join(collisionPath, "state.json")
	defer func() { stateFile = oldStateFile }()

	state := State{JobID: "test"}
	err = SaveState(state)
	if err == nil {
		t.Error("SaveState should have failed when parent directory is a file")
	}
}
