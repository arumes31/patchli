...
func TestHTTPPollingCaching(t *testing.T) {
	var callCount int
	var receivedPayloads []HeartbeatPayload
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/v1/poll" {
			...
			receivedPayloads = append(receivedPayloads, hb)

			w.WriteHeader(http.StatusOK)
			json.NewEncoder(w).Encode(CommandPayload{})
		}
	}))
	defer server.Close()
	...
	// Mock PackageManager to simulate changes
	mock := &configurableMockPM{}

	// Start polling in background
	go func() {
		// Use a very short ticker for testing if possible, but startHTTPPolling uses 30s.
		// We'll have to rely on the first execution which happens immediately.
		// Wait, startHTTPPolling loop starts with execution then selects on ticker.
		startHTTPPolling(ctx, host, "test-node", mock)
	}()
	...
	// Since we can't easily trigger the ticker, this test is limited.
	// But we can at least verify the first payload.
	if receivedPayloads[0].RebootNeeded != false {
		t.Errorf("Expected rebootNeeded false, got %v", receivedPayloads[0].RebootNeeded)
	}
}

type configurableMockPM struct {
	reboot bool
}

func (m *configurableMockPM) CheckUpdates(ctx context.Context) (updater.UpdateResult, error) {
	return updater.UpdateResult{}, nil
}
func (m *configurableMockPM) ApplyUpdates(ctx context.Context, packages []string) (updater.UpdateResult, error) {
	return updater.UpdateResult{}, nil
}
func (m *configurableMockPM) RebootRequired() bool                     { return m.reboot }
func (m *configurableMockPM) PreFlightCheck(ctx context.Context) error { return nil }
func (m *configurableMockPM) Cleanup(ctx context.Context) error        { return nil }
